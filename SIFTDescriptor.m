...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    feature = [];
    patch_mag_cell=cell(0,1);
    patch_theta_cell=cell(0,1);
    x = 1:num_histograms:patch_size;
    y = 1:num_histograms:patch_size;
    
    for a=1:pixelsPerHistogram
        for b=1:pixelsPerHistogram
            %1~4, 5~8, 9~12, 13~16 for both rows and columns
            patch_mag_cell=[patch_mag_cell patch_mag(x(a):x(a)+pixelsPerHistogram-1,y(b):y(b)+pixelsPerHistogram-1)];
            patch_theta_cell=[patch_theta_cell patch_theta(x(a):x(a)+pixelsPerHistogram-1,y(b):y(b)+pixelsPerHistogram-1)];
        end
    end
    
    
    for k=1:num_histograms * num_histograms
        [histogram, angles] = ComputeGradientHistogram(num_angles, patch_mag_cell{k}, patch_theta_cell{k});
        feature = [feature, histogram];
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for counti = 1:num_bins;
        max = counti * angle_step;
        min = max - angle_step;
        matrix = (gradient_angles >= min)&(gradient_angles <= max);
        %elements of matrix either 1 or 0
        product = gradient_magnitudes.*matrix; 
        histogram(counti) = sum(sum(product));
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    matrixd = angles(histogram == max(histogram));
    %elements of matrixd are either 0 or 1
    direction = matrixd(1);
