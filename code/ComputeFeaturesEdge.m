...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Skeleton of Canny reference: Rachmawan, Implementation of Canny Algorithm, 
%%https://de.mathworks.com/matlabcentral/fileexchange/46859-canny-edge-detection

%Below is the implementation of Canny Edge Detector on only img(:,:,1), same
%procedure for img(:,:,2), and img(:,:,3)

%Value for Thresholding (sample value from Rachmawan)
T_Low = 0.075;
T_High = 0.175;
%Gaussian Filter Coefficient (sample value from Rachmawan)
B = [2, 4, 5, 4, 2; 4, 9, 12, 9, 4;5, 12, 15, 12, 5;4, 9, 12, 9, 4;2, 4, 5, 4, 2 ];
B = 1/159.* B;
%Convolution of image by Gaussian Coefficient
A=conv2(img(:,:,1), B, 'same');
%Filter for horizontal and vertical direction (sample value from Rachmawan)
...
feature(:,:,1) = edge_final;
figure, imshow(edge_final);
%here we can see the result of using Canny Edge detector
end