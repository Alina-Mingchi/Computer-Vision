...
imgList = dir('./drone_sequential_images/*.png');
saveFileName = 'droimg.png';
...
for i = 1 : length(imgList),
    ...
    IMAGES{i} = imread(['./drone_sequential_images/' imgList(i).name]);
    %% Resize to make memory efficient
    if max(size(IMAGES{i})) > 900 || length(imgList) > 10,
...
for i = 1 : (length(imgList)-1),
    ...
    M = SIFTSimpleMatcher(DESCRIPTOR{i}, DESCRIPTOR{i+1}, 0.6);
    disp(size(M,1))
